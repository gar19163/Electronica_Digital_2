...
//-----------------I2C-----------------------------
        I2C_Master_Start();
        I2C_Master_Write(0x61); //Slave 2 con push y contador
        dig2 = I2C_Master_Read(0);
        I2C_Master_Stop();
        __delay_ms(200);        

        I2C_Master_Start();
        I2C_Master_Write(0x51); //Slave 1 ADC
        dig1 = I2C_Master_Read(0);
        I2C_Master_Stop();
        __delay_ms(200);

        I2C_Master_Start();
        I2C_Master_Write(0x80); //Sensonr 
...
//-----------------LCD-----------------------------------        
        ...
        Lcd_Write_String("ADC:  CONT:  TEMP:");//Imprimo texto
        
        //--------ADC-----------------------------------
        dig1 = dig1*0.0196;             //Convieto valor
        Lcd_Set_Cursor(2,1);            //Columna  fila 2
        sprintf(s,"%3.2fV",dig1);       //Selección de texto 
        Lcd_Write_String(s);            //Envio texto 
        //--------CONT-----------------------------------
        Lcd_Set_Cursor(2,6);            //Columna  fila 2
        sprintf(s,"%3.0fU",dig2);       //Selección de texto 
        Lcd_Write_String(s);            //Envio texto 
        //--------Temp-------------------------------
        dig3 = dig3-81;                 //Convieto valor
        Lcd_Set_Cursor(2,12);            //Columna  fila 2
        sprintf(s,"%3.1fC",dig3);        //Selección de texto 
        Lcd_Write_String(s);            //Envio texto 
    }
}
//-------------------------------------------------------
